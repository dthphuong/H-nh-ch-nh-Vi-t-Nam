...
function dateTimeToString(myDate) {
    ...
    return day.dd + '/' + day.mm + '/' + day.yyyy + " at " + day.hrs + ":" + day.min + ":" + day.sec + ' ' + day.ii;
}
...
exports.toISODateString = (myDate) => {
    ...
    return `ISODate`
    return ISODate(yyyy + '-' + mm + '-' + dd + 'T' + hrs + ':' + min + ':' + sec + '.000Z');
}
//#endregion
...
//#region Get getMonth of today
exports.getMonth = (myDate) => {
    var yyyy = new Date(myDate).getFullYear();
    var firstMonth = new Date(myDate).getMonth() + 1;

    if (firstMonth < 10) {
        firstMonth = '0' + firstMonth;
    }
    var lastMonth = new Date(myDate).getMonth() + 2;
    ...
    return {
        firstDate: new Date(yyyy + '-' + firstMonth),
        lastDate: new Date(new Date(yyyy + '-' + lastMonth).setDate(new Date(yyyy + '-' + lastMonth).getDate() - 1))
    };
}
...
exports.getTimeElapsed = (ptime) => {
    ...
    if (timeDiff < 0) {
        ...
        if (timeDiff < 10) return "Just now";
...
exports.toDateTimeID = (timestamp) => {
    return moment(timestamp).format('YYYYMMDD_HHmmss')
}

// Return milisecond of timeA - timeB
exports.minus = (timeA, timeB) => {
    return ((new Date(timeA)).getTime() - (new Date(timeB)).getTime()) / 1000
}

// Return milisecond of timeA
exports.msecOf = (timeA) => {
    return (new Date(timeA)).getTime()
}
